...
InputIterator CutOffset(InputIterator start, InputIterator const stop, uint32_t const offset)
{
  ...
  for (uint32_t distance = 0; start != stop; ++start)
  {
    ...
    {
      // Throw out this edge if (offst - distance) is greater than edgeLength / 2.
...
bool OpenLRDecoder::DecodeOne(LinearSegment const & segment, Index const & index, Graph & graph,
                              RoadInfoGetter & infoGetter, DecodedPath & path, v2::Stats & stat)
{
  ...
  // TODO(mgsergio): Make a concant iterator
  ...
  for (auto const & p : points)
    requiredRouteDistanceM += p.m_distanceToNextPoint;
